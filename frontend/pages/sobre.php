<?php
?>

<!DOCTYPE html>
<html lang="pt-br">
<head>
  <meta charset="UTF-8">
  <title>Sobre - Bonde da Esperança</title>
  <link rel="stylesheet" href="../css/style.css">
</head>
<body class="home-page">

  <header class="topo" style="justify-content: space-between; padding-right: 30px;">
    <div class="logo">Bonde da Esperança</div>

    <nav style="margin-left: auto;">
      <a href="../../index.php">Início</a>
      <a href="sobre.php">Sobre</a>
      <a href="login.php" class="adm">Admin</a>
    </nav>
  </header>

  <main class="home-container">
    <section class="lado-esquerdo">
      <h1>Sobre o Bonde da Esperança</h1>
      <p class="descricao-ong">
        O Bonde da Esperança é uma iniciativa social liderada por um professor de jiu-jitsu que, junto com voluntários, realiza a distribuição de refeições para moradores de rua sempre que possível.
      </p>
      <p class="descricao-ong">
        Nosso objetivo é levar alimento, carinho e apoio às pessoas em situação de rua, conectando quem pode ajudar com quem mais precisa. Cada saída é planejada com cuidado, respeitando datas, voluntários e segurança.
      </p>
      <p class="descricao-ong">
        Se você deseja contribuir, uma das melhores formas é se inscrever como voluntário ou apoiar o projeto com doações. Juntos podemos fortalecer a rede do bem.
      </p>
    </section>

    <section class="lado-direito">
      <div class="card-apoio">
        <h2>Como participar</h2>
        <p>As saídas são divulgadas no site e a inscrição é realizada diretamente na página inicial. Fique atento às datas e prazos.</p>
      </div>

      <div class="card-apoio">
        <h2>Contato</h2>
        <p>Para dúvidas ou apoio, envie uma mensagem no nosso instagram pelo botão abaixo.</p>
        <a href = "https://www.instagram.com/bondedaesperancaa/" target="_blank"><button class="btn-texto verde">Clique aqui</button></a>
      </div>
    </section>
  </main>

  <footer>Todos os direitos reservados</footer>

</body>
</html>
